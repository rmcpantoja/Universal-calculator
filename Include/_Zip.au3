...
;					t0nz modified in 2021 COM handler REMOVED (BUT already not called by other funcS)
;
; NOTES:
...
Func _IsFullPath($sPath)
	If StringInStr($sPath, ":\") Then ; original
		;If StringRegExp($sPath, '^([a-zA-Z]\:|\\\\[^\/\\:*?"<>|]+\\[^\/\\:*?"<>|]+)(\\[^\/\\:*?"<>|]+)+$') Then
		;pearl from http://www.autoitscript.com/forum/topic/116565-zip-udf-zipfldrdll-library/page-4#entry857293
		Return True
	Else
		Return False
	EndIf
